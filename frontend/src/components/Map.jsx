  ...
  useEffect(() => {
    ...
    map.current.on('load', () => {
      ...
      map.current.on('click', 'unclustered-point', (e) => {
        ...
        // Get carrier color based on type (1PL/2PL/3PL)
        const getCarrierColor = (type) => {
          const colors = {
            '1PL': '#3B82F6', // Blue
            '2PL': '#10B981', // Green
            '3PL': '#F59E0B'  // Orange
        ...
        const carrierColor = getCarrierColor(props.carrier_type);
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-top: 10px; padding-top: 10px; border-top: 1px solid #eee;">
                  <div style="background: #f0f7ff; padding: 8px; border-radius: 6px; border-left: 3px solid #4264fb;">
                    ...
                    <div style="font-size: 10px; color: #999; margin-top: 2px;">
                      1PL: ${onePlCount} | 2PL: ${twoPlCount} | 3PL: ${threePlCount}
                    </div>
                  </div>
                ...
                ${!isEmpty ? `
                  <div style="margin-top: 10px; padding: 10px; background: #f8f9fa; border-radius: 6px; border: 1px solid #dee2e6;">
                    ...
                    <div style="display: flex; justify-content: space-between;">
                      <div style="font-size: 11px; color: #333;">
                        <span style="display: inline-block; width: 8px; height: 8px; background: #F59E0B; border-radius: 50%; margin-right: 4px;"></span>
                        3PL Orders:
                      </div>
                      <div style="font-size: 11px; font-weight: bold; color: #F59E0B;">
      ...
      try {
        ...
        if (!bounds.isEmpty()) {
          map.current.fitBounds(bounds, {
            ...
            maxZoom: 12,
            minZoom: 8, // Prevent extreme zoom out for outlier destinations
            duration: 1000
          });
        }
      } catch (error) {
