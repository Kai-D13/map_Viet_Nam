              ...
              <div style={{
                ...
                {!selectedHub ? (
                  ...
                  filteredDestinations.map(dest => {
                    // Get carrier colors based on type (1PL/2PL/3PL)
                    const getCarrierColors = (type) => {
                      const colors = {
                        '1PL': { color: '#3B82F6', bg: '#dbeafe' },
                        '2PL': { color: '#10B981', bg: '#dcfce7' },
                        '3PL': { color: '#F59E0B', bg: '#fef3c7' }
                      };
                      return colors[type] || { color: '#999999', bg: '#f5f5f5' };
                    };
                    const carrierColors = getCarrierColors(dest.carrier_type);
                    const carrierColor = carrierColors.color;
                    const carrierBg = carrierColors.bg;
                ...
                <div style={{ maxHeight: '200px', overflowY: 'auto' }}>
                  {calculatedRoutes.map((route, idx) => {
                    ...
                    const carrierBadgeStyle = {
                      ...
                      fontWeight: 'bold',
                      marginLeft: '6px',
                      backgroundColor: route.carrier_type === '1PL' ? '#dbeafe' : route.carrier_type === '2PL' ? '#dcfce7' : route.carrier_type === '3PL' ? '#fef3c7' : '#e2e3e5',
                      color: route.carrier_type === '1PL' ? '#1e40af' : route.carrier_type === '2PL' ? '#065f46' : route.carrier_type === '3PL' ? '#92400e' : '#383d41'
                    };
